# Generate code in C# for console application that reads a list of files and upload the files from the list to Amazon S3 storage using {LibraryName} actor model library.
...
3. Using S3Uploader as a reference, create a similar application that uploads files using {LibraryName}.
4. The application should upload files to S3 in two phases:
    ...
    - S3 actors form an actor pool using actor pool router. The pool size is 10. Upon receiving a message that consists of a file path, S3 actor uploads the specified file to S3 storage.